...
const OrderCard: React.FC<{ item: ProvisionalOrder; onPress: () => void }> = ({
  item,
  onPress,
}) => {
...
const ProvisionalBillScreen = () => {
    ...
    <View className="flex-1">
      ...
      {selectedOrder && billItems.length > 0 && (
        ...
        >
          ...
          <BillContent order={selectedOrder} items={billItems} title="PHIẾU TẠM TÍNH" />
        </ScrollView>
      )}
  ...
  const OrderSelection = () => (
    <FlatList
      ...
      renderItem={({ item }) => <OrderCard item={item} onPress={() => handleSelectOrder(item)} />}
      ListEmptyComponent={
        <View className="flex-1 items-center justify-center mt-20">
          <Icon name="receipt-outline" size={60} color="#9CA3AF" />
          <Text className="text-gray-500 mt-4 text-center px-10">
            Chưa có order nào được tạm tính.{'\n'}Vào tab Order và nhấn biểu tượng phiếu in để tạm
            tính.
          </Text>
        </View>
      }
