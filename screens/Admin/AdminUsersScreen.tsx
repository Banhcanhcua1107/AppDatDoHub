...
export default function AdminUsersScreen() {
  ...
  const handleSaveUser = async () => {
    if (!selectedUser || !formData.full_name.trim()) {
      Alert.alert('Lỗi', 'Vui lòng điền tên nhân viên');
      return;
    }
    try {
  ...
  const handleDeleteUser = (userId: string) => {
    Alert.alert(
      'Xóa nhân viên',
      'Xóa người dùng là một hành động nguy hiểm. Vui lòng thực hiện trong trang quản trị Supabase.',
      [{ text: 'Đã hiểu' }]
    );
  };
              ...
              <>
                ...
                <ScrollView style={styles.modalScroll}>
                  <View style={styles.section}>
                    <Text style={styles.sectionLabel}>Họ tên</Text>
                    <TextInput
                      style={styles.input}
                  ...
                  <View style={styles.section}>
                    ...
                    {(['nhan_vien', 'bep', 'thu_ngan', 'admin'] as const).map((role) => (
                      ...
                      >
                        <View style={[styles.roleRadio, formData.role === role && styles.roleRadioSelected]}>
                          {formData.role === role && <View style={styles.roleRadioDot} />}
                        </View>
...
const styles = StyleSheet.create({
  ...
  searchSection: { backgroundColor: 'white', paddingHorizontal: 16, paddingVertical: 12, flexDirection: 'row', alignItems: 'center', borderBottomWidth: 1, borderBottomColor: '#E5E7EB' },
  searchIcon: { marginRight: 8 },
  searchInput: { flex: 1, fontSize: 14, color: '#1F2937' },
  listContent: { paddingHorizontal: 16, paddingVertical: 12 },
  userRow: { backgroundColor: 'white', borderRadius: 12, marginBottom: 12, elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 4 },
  userContent: { paddingHorizontal: 14, paddingVertical: 12, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  userLeft: { flex: 1 },
  userName: { fontSize: 15, fontWeight: '600', color: '#1F2937' },
  userEmail: { fontSize: 12, color: '#9CA3AF', marginTop: 4 },
  ...
  userActions: { flexDirection: 'row', justifyContent: 'center', borderTopWidth: 1, borderTopColor: '#E5E7EB', paddingVertical: 8, gap: 12 },
  actionBtn: { padding: 8 },
  ...
  input: { backgroundColor: '#F3F4F6', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, fontSize: 14, color: '#1F2937', borderWidth: 1, borderColor: '#E5E7EB' },
  roleOption: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, paddingHorizontal: 12, backgroundColor: '#F3F4F6', borderRadius: 8, marginBottom: 8, borderWidth: 2, borderColor: 'transparent' },
  ...
  saveBtn: { flex: 1, backgroundColor: '#3B82F6', borderRadius: 8, paddingVertical: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 },
  saveBtnText: { color: '#fff', fontWeight: '600', fontSize: 14 },
  cancelBtn: { flex: 1, backgroundColor: '#fff', borderRadius: 8, paddingVertical: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, borderWidth: 1, borderColor: '#3B82F6' },
  cancelBtnText: { color: '#3B82F6', fontWeight: '600', fontSize: 14 },
});