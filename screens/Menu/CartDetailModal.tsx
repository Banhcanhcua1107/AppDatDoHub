...
const CartDetailModal: React.FC<CartDetailModalProps> = ({
  visible, onClose, cartItems, onUpdateQuantity, onRemoveItem, onClearCart
}) => {
    ...
    return (
      <View className="flex-row items-center mb-4 pb-4 border-b border-gray-200">
        ...
        <View className="flex-1">
          <Text className="text-base font-bold text-gray-800">{customizations.name || 'Món đã bị xoá'}</Text>
          ...
          {customizations.note && (<View className="bg-yellow-50 p-2 rounded-md mt-2"><Text className="text-sm text-yellow-800 italic">Ghi chú: {customizations.note}</Text></View>)}
          <Text className="text-base font-semibold text-red-500 mt-2">{item.unit_price.toLocaleString('vi-VN')}đ</Text>
        </View>
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalContainer}>
          <View className="flex-row items-center justify-between pb-3 border-b border-gray-200">
            <TouchableOpacity onPress={handleClearCart} className="p-2"><Text className="text-blue-500">Xoá tất cả</Text></TouchableOpacity>
            <Text className="text-xl font-bold">Giỏ hàng</Text>
            <TouchableOpacity onPress={onClose} className="p-2"><Icon name="close" size={28} color="#333" /></TouchableOpacity>
          </View>
          <FlatList data={cartItems} renderItem={renderCartItem} keyExtractor={item => String(item.id)}
            contentContainerStyle={{ paddingTop: 16, paddingBottom: 20 }}
            ListEmptyComponent={() => <View className="items-center justify-center h-40"><Text className="text-gray-500">Giỏ hàng của bạn đang trống</Text></View>}
          />
        </View>
      </View>
    </Modal>
...
const styles = StyleSheet.create({
  ...
  modalContainer: { backgroundColor: 'white', height: '75%', borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 16 },
});
