...
export type TableSelectionParams = {
  ...
  action: 'transfer' | 'merge' | 'split' | 'group';
  sourceRoute: string;
  sourceTable?: { id: string; name: string; orderId?: string };
  maxSelection?: number;
};
...
const TableItem: React.FC<{
  ...
  return (
    ...
    >
      ...
      <Text style={[styles.tableStatus, isOccupied && styles.lightText]}>
        {isSourceTable ? 'Bàn gốc' : item.status}
      </Text>
    </TouchableOpacity>
  );
};
...
const handleGroupTables = async (orderId: string, targetTables: Table[]) => {
  if (!orderId || targetTables.length === 0)
    throw new Error('Thiếu thông tin order hoặc bàn đích.');
...
const handleMergeOrder = async (sourceOrderId: string, targetTables: Table[]) => {
  if (!sourceOrderId || targetTables.length === 0)
    throw new Error('Thiếu thông tin order nguồn hoặc bàn đích.');
...
const TableSelectionScreen = ({ route, navigation }: Props) => {
    ...
    switch (action) {
      case 'group':
        if (!sourceTable.orderId) {
          Alert.alert('Lỗi', 'Không tìm thấy order của bàn gốc.');
          return;
        }
        Alert.alert(
          'Xác nhận Gộp Bàn',
          `Bạn có chắc muốn gộp các bàn [${selectedTablesInfo}] vào chung một order với nhóm bàn ${sourceTable.name}?`,
          [
            { text: 'Hủy' },
            {
              text: 'Xác nhận',
      ...
      headerRight: () => (
        <TouchableOpacity onPress={handleConfirmSelection} disabled={selectedTableIds.size === 0}>
          <Text
            style={[
              styles.headerButton,
              selectedTableIds.size === 0 && styles.headerButtonDisabled,
            ]}
          >
            Xong
          </Text>
  ...
  const handleSelectTable = (tableId: string) => {
    ...
    if (action === 'transfer' && targetTable.status !== 'Trống') {
      Alert.alert('Không hợp lệ', 'Chuyển bàn chỉ có thể chọn bàn còn trống.');
      return;
    }
    if (action === 'merge' && targetTable.status !== 'Đang phục vụ') {
      Alert.alert('Không hợp lệ', 'Gộp order phải chọn bàn đang phục vụ.');
      return;
    }
    if (action === 'group' && targetTable.status !== 'Trống') {
      Alert.alert('Không hợp lệ', 'Gộp bàn chỉ có thể chọn bàn còn trống.');
      return;
    }
    if (mode === 'single') {
      newSelection.clear();
