...
const SplitItemRow: React.FC<{
  ...
  return (
    <View style={styles.itemContainer}>
      ...
      <Image source={{ uri: item.image || placeholderImage }} style={styles.itemImage} />
      <View style={styles.itemInfoContainer}>
        <Text style={styles.itemName} numberOfLines={2}>
          {item.name}
        </Text>
        <Text style={styles.itemPrice}>
          Tổng: {item.originalQuantity} • {item.unit_price.toLocaleString('vi-VN')}đ/món
        </Text>
      </View>
  ...
  return (
    <View style={styles.flex1}>
      ...
      <FlatList
        data={items}
        renderItem={({ item }) => (
          <SplitItemRow item={item} onUpdateQuantity={handleUpdateQuantity} />
        )}
        keyExtractor={(item) => item.id.toString()}
        contentContainerStyle={{ padding: 16 }}
        ListEmptyComponent={
          <Text style={{ textAlign: 'center', marginTop: 50 }}>Order gốc không có món nào.</Text>
        }
      />
      <View style={[styles.bottomBar, { paddingBottom: insets.bottom > 0 ? insets.bottom : 20 }]}>
        ...
        >
          {isSubmitting ? (
            ...
            <Text style={styles.confirmButtonText}>Xác nhận tách</Text>
          )}
        </TouchableOpacity>
      </View>
    </View>
