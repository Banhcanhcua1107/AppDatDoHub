...
import {
  ...
  Image, FlatList, ScrollView, ActivityIndicator, Alert, InteractionManager, Modal, Pressable
} from 'react-native';
...
interface MenuItemFromDB {
  id: string; name: string; description: string | null; price: number; image_url: string | null; categoryId: string;
}
...
export interface CartItemFromDB {
    id: number; table_id: number; menu_item_id: number; quantity: number; total_price: number; unit_price: number;
    customizations: { name: string; image: string; note?: string; size: { name: string; price: number }; sugar: { name: string; price: number }; toppings: { name: string; price: number }[]; };
}
        ...
        <Pressable style={styles.filterModalContainer}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              return (
                <TouchableOpacity style={styles.filterItem} onPress={() => onToggleCategory(item.id)}>
                  <Icon name={isSelected ? "checkbox" : "square-outline"} size={24} color={isSelected ? "#3B82F6" : "#6B7280"} />
...
const MenuScreen = ({ route, navigation }: MenuScreenProps) => {
  ...
  const handleAddToCart = async (itemWithOptions: CartItem) => {
    try {
        ...
        const { data: existingItem, error: findError } = await supabase.from('cart_items').select('id, quantity, total_price').eq('unique_id', uniqueId).single();
        ...
        if (existingItem) {
            ...
            await supabase.from('cart_items').update({ quantity: newQuantity, total_price: newTotalPrice }).eq('id', existingItem.id).throwOnError();
        } else {
            await supabase.from('cart_items').insert({
                table_id: tableId, menu_item_id: itemWithOptions.menuItemId, quantity: itemWithOptions.quantity,
                unit_price: itemWithOptions.totalPrice / itemWithOptions.quantity, total_price: itemWithOptions.totalPrice,
                customizations: { size: itemWithOptions.size, sugar: itemWithOptions.sugar, toppings: itemWithOptions.toppings, note: itemWithOptions.note, name: itemWithOptions.name, image: itemWithOptions.image },
                unique_id: uniqueId
            }).throwOnError();
        }
        await fetchData(); 
  ...
  const handleUpdateQuantity = async (cartItemId: number, newQuantity: number) => {
    try {
      if (newQuantity <= 0) { await handleRemoveItem(cartItemId); return; }
  ...
  const handleGoBackWithConfirmation = () => {
    if (cartItems.length > 0) {
      Alert.alert("Thoát và hủy các món đã chọn?", "Bạn có chắc muốn thoát và xóa các món mới chọn?",
        [ { text: "Ở lại" }, { text: "Thoát & Xóa", style: "destructive", onPress: async () => { await handleClearCart(); navigation.goBack(); } } ]
      );
  ...
  const itemsToDisplay = (() => {
    ...
    if (isFiltering) {
      potentialItems = potentialItems.filter(item => selectedFilterCategoryIds.includes(item.categoryId));
    }
    ...
    if (isSearching) {
      potentialItems = potentialItems.filter(item => {
        ...
        return normalizedItemName.includes(normalizedQuery) || itemAcronym.includes(normalizedQuery);
      });
    }
    ...
    <View style={styles.flex1}>
      ...
      <View style={{ paddingTop: insets.top + 20, backgroundColor: 'white' }} className="px-4 pb-3">
        ...
        <View style={styles.searchRow}>
          ...
          <TouchableOpacity style={styles.filterTrigger} onPress={() => setFilterModalVisible(true)}>
        ...
        ListEmptyComponent={
            <View style={{marginTop: 50, alignItems: 'center'}}>
                <Text style={{color: 'gray', fontSize: 16}}>
                    {searchQuery || isFilterActive ? 'Không tìm thấy món nào phù hợp' : 'Danh mục này chưa có món'}
                </Text>
            </View>
        }
      />
