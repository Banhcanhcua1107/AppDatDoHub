...
const HomeScreen = ({ navigation }: HomeScreenProps) => {
  ...
  const fetchTables = React.useCallback(async () => {
    setLoading(true);
    const { data, error } = await supabase.from('tables').select('id, name, status').order('id', { ascending: true });
    if (error) {
    ...
    React.useCallback(() => {
      ...
      const channel = supabase
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'tables' }, 
            payload => { console.log('Table change detected, refetching.'); fetchTables(); }
        ).subscribe();
...
const styleSheet = StyleSheet.create({
  ...
  fab: { position: 'absolute', right: 20, width: 60, height: 60, borderRadius: 30, backgroundColor: '#3B82F6', justifyContent: 'center', alignItems: 'center', elevation: 8, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 4, }
});
