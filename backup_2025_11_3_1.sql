...
BEGIN
    ...
    LOOP
        ...
        INSERT INTO public.stock_history (ingredient_id, quantity_change)
        VALUES (po_item.ingredient_id, po_item.quantity);
    END LOOP;
END;
$$;
...
DECLARE
    recipe_item RECORD;
BEGIN
    -- Lặp qua tất cả nguyên liệu trong công thức của món ăn vừa được đặt
    FOR recipe_item IN
        SELECT
            mii.ingredient_id,
            mii.quantity_required * NEW.quantity AS total_to_deduct -- Nhân với số lượng món bán ra
        FROM public.menu_item_ingredients mii
        WHERE mii.menu_item_id = NEW.menu_item_id
    LOOP
        -- Cập nhật (trừ đi) số lượng tồn kho của nguyên liệu đó
        UPDATE public.ingredients
        SET stock_quantity = stock_quantity - recipe_item.total_to_deduct
