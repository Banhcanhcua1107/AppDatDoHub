...
BEGIN
    ...
    SELECT JSON_BUILD_OBJECT(
        'todayRevenue', COALESCE(SUM(CASE 
            WHEN o.status IN ('paid', 'completed', 'closed') THEN COALESCE((
        ...
        'paidOrdersCount', COUNT(DISTINCT CASE WHEN o.status IN ('paid', 'completed', 'closed') THEN o.id END)
    ) INTO v_stats
    FROM orders o
    ...
    FROM (
        SELECT 
            m.id,
            m.name,
            COUNT(oi.id)::INTEGER as quantity,
    ...
    -- Get recent activities WITH TABLE NAMES
    SELECT JSON_AGG(ROW_TO_JSON(t)) INTO v_activities
    FROM (
        SELECT 
            o.id,
            COALESCE(t.name, 'Bàn N/A') as name,
            t.id as table_id,
            o.status,
            o.created_at,
            COALESCE((
